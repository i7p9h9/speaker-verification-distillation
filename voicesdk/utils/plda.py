...
class PLDA(object):
    ...
    def save(self, path):
        ...
        # save_dir = os.path.join(path, "plda")
        # os.makedirs(save_dir)
        # for k, v in vars(self).items():
        #     np.save(os.path.join(save_dir, "{}.npy".format(k)), v)

        # np.save(os.path.join(save_dir, "mean.npy"), self.mean)
        # np.save(os.path.join(save_dir, "transform.npy"), self.transform)
        # np.save(os.path.join(save_dir, "psi.npy"), self.psi)
        # np.save(os.path.join(save_dir, "dim.npy"), self.dim)
        # np.save(os.path.join(save_dir, "normalize_length.npy"), self.normalize_length)
        # np.save(os.path.join(save_dir, "simple_length_norm.npy"), self.simple_length_norm)
        # np.save(os.path.join(save_dir, "offset.npy"), self.offset)

    def load(self, path):
        loaded_state = np.load(path, allow_pickle=True)[0]
        self.__dict__ = loaded_state
