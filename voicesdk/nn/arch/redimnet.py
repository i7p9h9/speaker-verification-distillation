...
# import wespeaker.models.head as heads
# import wespeaker.models.features as features
# import wespeaker.models.features_tf as features_tf
...
class ConvBlock2d(nn.Module):
    def __init__(
        ...
        res_basic_block = functools.partial(ResBasicBlock,
            inc=c,
            outc=c,
            num_freq=f,
            stride=1,
        ...
        **kwargs
        ...
        if block_type == 'fc':
            ...
            self.tcm = nn.Sequential(*[ConvNeXtLikeBlock(
                hC, dim=1, kernel_sizes=[7, 15, 31], Gdiv=1, padding='same', causal=causal
            ) for i in range(4)])
            ...
            # Just large kernel size conv like in convformer
            self.tcm = nn.Sequential(*[ConvNeXtLikeBlock(
                hC, dim=1, kernel_sizes=[3, 7, 15], Gdiv=1, padding='same', causal=causal
            ) for i in range(3)])

        elif block_type == 'convx2':
            # Just large kernel size conv like in convformer
            self.tcm = nn.Sequential(*[ConvNeXtLikeBlock(
                hC, dim=1, kernel_sizes=[7], Gdiv=1, padding='same', causal=causal
            ) for i in range(2)])

        elif block_type == 'conv_fov_80':
            # Just large kernel size conv like in convformer
            self.tcm = nn.Sequential(*[ConvNeXtLikeBlock(
                hC, dim=1, kernel_sizes=[7, 21], Gdiv=1, padding='same', causal=causal
            ) for i in range(4)])
        ...
        #     )
            ...
            pre_layers = []
            if causal:
                pre_layers.append(CausalPaddingNd(
                    dim=1, kernel_size=pos_ker_sz, dilation=1
                ))
            self.tcm = nn.Sequential(
                *pre_layers,
...
def resolve_module_conf(module_config):
    if isinstance(module_config,dict):
        module = eval(module_config["type"])(
            *module_config.get("args",[]),
...
# Final prepooling num frequencies:
# 72 MelBanks -> 72 // 2 // 2 // 2 = 9
def ReDimNet_Lv0_c32f72o9x15ms_rnfwse_catt(embed_dim=192, feat_dim=72, feat_type='pt', pooling_func='ASTP', emb_bn=False):
    assert feat_dim == feat_dim # Dummy string only for back-compatibility with wespeaker
    return ReDimNetWrap(
        F = 72, 
        C = 32,
...
# Final prepooling num frequencies:
# 72 MelBanks -> 72 // 2 // 2 // 2 = 9
def ReDimNet_Lv0_c32f72o9x15ms_rnfwse_att(embed_dim=192, feat_dim=72, feat_type='pt', pooling_func='ASTP', emb_bn=False):
    assert feat_dim == feat_dim # Dummy string only for back-compatibility with wespeaker
    return ReDimNetWrap(
        F = 72, 
        C = 32,
