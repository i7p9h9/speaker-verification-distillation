...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.causal:
            # Create a [tgt_len x src_len] mask where positions [i, j] with j > i are set to True
            mask = torch.triu(torch.ones(tgt_len, src_len, device=hidden_states.device), diagonal=1).bool()
            # Reshape attn_weights to [bsz, num_heads, tgt_len, src_len] to apply mask
            attn_weights = attn_weights.view(bsz, self.num_heads, tgt_len, src_len)
            attn_weights = attn_weights.masked_fill(mask.unsqueeze(0).unsqueeze(0), float("-inf"))
            # Reshape back to [bsz*num_heads, tgt_len, src_len]
            attn_weights = attn_weights.view(bsz * self.num_heads, tgt_len, src_len)
...
class FeedForward(nn.Module):
    ...
    # def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
    #     """
    #     Forward pass for the FeedForward module with explicit reshaping.

    #     Args:
    #         hidden_states (Tensor): Input tensor of shape [batch_size, seq_length, hidden_size]
    ...
    #     # Reshape from [batch_size, seq_length, hidden_size] to [batch_size * seq_length, hidden_size]
    #     hidden_states = hidden_states.view(-1, hidden_size)  # [B * S, H]

    #     # Intermediate Dense Layer
    #     hidden_states = self.intermediate_dense(hidden_states)  # [B * S, Intermediate]
    #     hidden_states = self.intermediate_act_fn(hidden_states)  # Activation
    #     hidden_states = self.intermediate_dropout(hidden_states)  # Dropout

    #     # Output Dense Layer
    #     hidden_states = self.output_dense(hidden_states)        # [B * S, H]
    #     hidden_states = self.output_dropout(hidden_states)      # Dropout

    #     # Reshape back to [batch_size, seq_length, hidden_size]
    #     hidden_states = hidden_states.view(batch_size, seq_length, hidden_size)  # [B, S, H]
